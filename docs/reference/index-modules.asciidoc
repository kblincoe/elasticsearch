...
[[index-codec]] `index.codec`::
    ...
    NOTE: `index.codec` cannot be set directly on an index. It must first be set on the node level. Then, the index
    in question needs to be closed, updated, reopened and force merged. The same can be achieved when moving an
    index between nodes by force merging the moved index.
